...
public class UcAlgo 
{
	...
	public double[] ucAlgoDwt(double[] iInput) throws Exception
	{
		 ...
		 */
			...
			mMatrixFunctions.filtfilt_Sos(aInput, SignalProcConstants.UC_HIGH_SOS, SignalProcConstants.UC_HIGH_GAIN,
					SignalProcConstants.UC_HIGH_Z, SignalProcConstants.UC_HIGH_ORDER);

			mMatrixFunctions.filtfilt_Sos(aInput, SignalProcConstants.UC_LOW_SOS, SignalProcConstants.UC_LOW_GAIN,
					SignalProcConstants.UC_LOW_Z, SignalProcConstants.UC_LOW_ORDER);
			...
			for (int i = 0; i< SignalProcConstants.NO_OF_PRINT_VALUES; i++ ) {
				...
				if (SignalProcUtils.ucCounter < SignalProcConstants.UC_WINDOW) {
					...
					aUC_Energy[i] = sum / SignalProcUtils.ucCounter;
				}
				else {
					...
					SignalProcUtils.ucCounter++;
					double sum = 0;
					for (int z = 0; z< SignalProcConstants.UC_WINDOW; z++) {
						sum = sum + SignalProcUtils.ucEnergyTemp.get(z);
					}
					aUC_Energy[i] = sum / SignalProcConstants.UC_WINDOW;
				}
				
//				if (SignalProcUtils.ucCounter == 0) {
//					SignalProcUtils.ucCounter++;
//					aUC_Energy[i] = aMedianEnergy;
//					SignalProcConstants.ucEnergyTemp.add(aMedianEnergy);
//					SignalProcConstants.ucPreviousEnergy = aUC_Energy[i];
//				}
//				else if (SignalProcConstants.ucCounter < SignalProcConstants.UC_WINDOW) {
//					SignalProcConstants.ucCounter++;
//					aUC_Energy[i] = ( (SignalProcConstants.ucPreviousEnergy*(SignalProcConstants.ucCounter-1) + aMedianEnergy) / SignalProcConstants.ucCounter);
//					SignalProcConstants.ucEnergyTemp.add(aMedianEnergy);
//					SignalProcConstants.ucPreviousEnergy = aUC_Energy[i];
//				}
//				else {
//					SignalProcConstants.ucCounter++;
//					aUC_Energy[i] = ( (SignalProcConstants.ucPreviousEnergy*(SignalProcConstants.UC_WINDOW) + aMedianEnergy - SignalProcConstants.ucEnergyTemp.removeFirst()) / SignalProcConstants.UC_WINDOW);
//					SignalProcConstants.ucEnergyTemp.add(aMedianEnergy);
//
//				}
