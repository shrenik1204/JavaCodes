...
//        double aDelta1 = SignalProcConstants.QRS_RR_VAR_Continuous;
    ...
    public Object[] qrsConcatenated(int[] iQRS,int[] iQrsM) throws Exception {
        ...
        iQRS = new int[iQRSlist.size()];
        for (int i = 0; i < iQRS.length; i++) {
            iQRS[i] = iQRSlist.get(i);
        }
        if (iQRS.length > 0) {
            if (iQRS[iQRS.length - 1] > iQRSLast) {
                ...
                double aNoRR = SignalProcConstants.QRS_NO_RR_MEAN - 2;
                // double aLowPerc = SignalProcConstants.QRS_RRLOW_PERC;
                // double aHighPerc = SignalProcConstants.QRS_RRHIGH_PERC;
                double aDelta = SignalProcConstants.QRS_RR_VAR_Concat;
...
//                    if(SignalProcUtils.lastvalidRRMeanFetal < 300){
//                        aRRLowTh = 1 / (1 / aRRMean + aDelta);
//                        aRRHighTh = 1 / (1 / aRRMean - aDelta);
//                    }else{
//                        aRRLowTh = 1 / (1 / SignalProcUtils.lastvalidRRMeanFetal + SignalProcConstants.QRS_RR_VAR_Concat);
//                        aRRHighTh = 1 / (1 / SignalProcUtils.lastvalidRRMeanFetal - SignalProcConstants.QRS_RR_VAR_Concat);
//                    }
                    aRRLowTh = 1 / (1 / aRRMean + aDelta);
                    aRRHighTh = 1 / (1 / aRRMean - aDelta);
                    if(aRRLowTh < SignalProcConstants.FQRS_RR_LOW_TH){
